...
class OrderExporter extends Exporter
{
    ...
    public static function getColumns(): array
    {
        ...
        if ($isAdmin) {
            $columns[] = ExportColumn::make('service_cost_snapshot')
                ->label('Costo Base (Gasto)');
            $columns[] = ExportColumn::make('service_price_snapshot')
                ->label('Precio de Lista (Valor Real)');
            $columns[] = ExportColumn::make('profit')
                ->label('Utilidad')
                ->state(fn (Order $record): float => $record->price_at_purchase - $record->service_cost_snapshot);
            $columns[] = ExportColumn::make('discount')
                ->label('Descuento')
